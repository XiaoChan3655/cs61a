...
def inc_subseqs(s):             #???
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = [[s[0]]]
            b = s[1:] 
            return a + inc_subseqs(b)
    return subseq_helper( s , max(s) if s else 0)
    """
...
def deep_len(lnk):
    ...
    if lnk==Link.empty:
        return 0
    elif isinstance(lnk, Link) is False:
        return 1
    else:
