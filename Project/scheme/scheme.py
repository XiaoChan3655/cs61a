...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        print(scheme_eval(expressions.first, env, True))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        #if formals is nil:
        #    return new_env
        list_formals, list_vals = pair_to_list(formals), pair_to_list(vals)
        #print('debug', list_formals)
        for formal, val in zip(list_formals, list_vals):
            new_env.bindings[formal] = val
        return new_env
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #print('debug', expressions.rest.__repr__)
        ...
        env.bindings[target.first] = func
        return target.first 
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    """
    list_expr = pair_to_list(expressions)
    if list_expr == []:
        return True
    ...
    if expressions is nil:
        ...
        if is_false_primitive(first_expr):  # The first expression is False
            return False
        elif is_true_primitive(first_expr):  # The first expression is True
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    list_bind = pair_to_list(bindings)
    for bind in list_bind:
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            return Thunk(expr, env)                            # return will terminate the recursive, so if-form need set tail to be true
    ...
    while True:
        try:
            ...
            while src.more_on_line:
                ...
                result = scheme_eval(expression, env)   # tail is false when called in the first time
