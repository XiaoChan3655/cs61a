...
class Insect:
    ...
    def remove_from(self, place):
        if isinstance(self, QueenAnt) is False:      
            self.place = None
        elif self.true_queen is False:
            self.place = None
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant == None and isinstance(self, ContainerAnt) is False:
                place.ant.contained_ant = self
            elif isinstance(self, ContainerAnt) and isinstance(place.ant, ContainerAnt) is False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        print('debug', self.min_range)
        next_place = self.place
        distance = 0
        while (next_place != beehive):         # the entrance of tunnel_0_9 is beehive, but the exit of beehive is not tunnel_0_9 !!!!
            if (next_place.bees != []) & (self.min_range <= distance <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        Ant.reduce_armor(self, amount)     # a test case modify the Ant.reduce_armor not Insect.armor
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        while last_place is not None:
            count += 1
        self.double_index = [0 for _ in range(count)]
        """
        # END Problem EC
        ...
        if self.true_queen == False:
            ...
            while last_place is not None:
                if last_place.ant is not None:
                    if hasattr(last_place.ant, 'doubled') is False:
                        last_place.ant.doubled = True
                        last_place.ant.damage = 2 * last_place.ant.damage
    ...
    def insects_in_front(self, beehive):
        ...
        if self.place.bees is not None:
            for bee in self.place.bees:
                    insects[bee] = 0 """
        last_place = self.place  
        distance = 0
        while last_place is not beehive:
            if distance == 0 and isinstance(last_place.ant, ContainerAnt):
                insects[last_place.ant] = distance
            elif distance > 0 and last_place.ant is not None:
                insects[last_place.ant] = distance
            if last_place.bees is not None:
                for bee in last_place.bees:
                    insects[bee] = distance
            distance += 1
            last_place = last_place.entrance
        return insects
        # END Problem Optional 5
